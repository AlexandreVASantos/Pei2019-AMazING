...
def consumer():
	consumer = KafkaConsumer('numtest', bootstrap_servers=['192.168.85.228:9092'], auto_offset_reset='earliest', enable_auto_commit=True, group_id='my_group', value_deserializer=lambda x:json.loads(x.decode('utf-8')))
	count = 0
...
def auth(request):
	...
	if user is not None:
		login(request,user)
		refresh_grid()
		producer = KafkaProducer(bootstrap_servers=['192.168.85.228:9092'],value_serializer=lambda x: dumps(x).encode('utf-8'))	#kafka stuff, sends  value to insert into database
...
def postAccessP(request):
	...
	if( verify_Channel(request,channel) and verify_passw(request,data["APPW"]) and verify_Range(request,data["RangeStart"],data["RangeEnd"]) and verify_IP(request,data["RangeStart"]) and verify_IP(request,data["RangeEnd"]) and verify_IP(request,data["DFGateway"])):
		...
		producer = KafkaProducer(bootstrap_servers=['192.168.85.228:9092'],value_serializer=lambda x: dumps(x).encode('utf-8'))
		curdate = datetime.datetime.today().strftime('[%d/%B/%Y %H:%M:%S]')
		username='username'
